...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a SMC address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no SMC: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert smc:// to smc:
    //
    //    Cannot handle this later, because smc:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open smc.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SMC.lnk";
}
...
{
    return GetAutostartDir() / "smc.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a smc.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the smc app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add smc app to startup item list
