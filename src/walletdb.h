...
// Copyright (c) 2016-2018 The SMC developers
...
public:
    ...
    bool WriteZSMCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSMCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSMCSeed_deprecated(uint256& seed);
    bool EraseZSMCSeed();
    bool EraseZSMCSeed_deprecated();

    bool WriteZSMCCount(const uint32_t& nCount);
    bool ReadZSMCCount(uint32_t& nCount);
