...
CzSMCWallet::CzSMCWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSMCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSMCSeed_deprecated()) {
                    LogPrintf("%s: Updated zSMC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzSMCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSMCCount(nCountLastUsed);
    else if (!walletdb.ReadZSMCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSMCWallet::Lock()
{
    seedMaster = 0;
}

void CzSMCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSMCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSMC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSMCWallet::LoadMintPoolFromDB()
{
...
void CzSMCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSMCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSMCWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzSMCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSMC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzSMCWallet::SeedToZSMC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSMCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSMCWallet::GenerateDeterministicZSMC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSMCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSMCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
