...
#ifndef SMC_ZSMCWALLET_H
#define SMC_ZSMCWALLET_H
...
public:
    CzSMCWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZSMC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZSMC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //SMC_ZSMCWALLET_H
