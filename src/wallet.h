...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 SMC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zSMC send
enum ZerocoinSpendStatus {
    ZSMC_SPEND_OKAY = 0,                            // No error
    ZSMC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZSMC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZSMC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZSMC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZSMC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZSMC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZSMC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZSMC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZSMC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZSMC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZSMC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZSMC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZSMC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZSMC_SPENT_USED_ZSMC = 14,                      // Coin has already been spend
    ZSMC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZSMC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZSMCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzSMCWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzSMCTracker> zpivTracker;
    ...
    void setZWallet(CzSMCWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzSMCTracker>(new CzSMCTracker(strWalletFile));
    }

    CzSMCWallet* getZWallet() { return zwalletMain; }
    ...
    /** zSMC reset */
    boost::signals2::signal<void()> NotifyzSMCReset;
