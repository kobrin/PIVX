...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zpivTracker = unique_ptr<CzSMCTracker>(new CzSMCTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZSMC_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZSMC_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
