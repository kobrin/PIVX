...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "smc.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "smcd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the SMC and zSMC money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 50472, 50474));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in SMC/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, smc, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all SMC specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-pivstake=<n>", strprintf(_("Enable or disable staking functionality for SMC inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zpivstake=<n>", strprintf(_("Enable or disable staking functionality for zSMC inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:50472"));
    ...
    strUsage += HelpMessageOpt("-backupzpiv=<n>", strprintf(_("Enable automatic wallet backups triggered after each zSMC minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zpivbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zSMC backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizesmcamount=<n>     " + strprintf(_("Keep N SMC anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The SMC Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that SMC is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. SMC Core is shutting down."));
...
#endif
    // Make sure only a single SMC process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SMC Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //SMC: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //SMC specific: zerocoin and spork DB's
                ...
                // SMC: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZSMCMinted();
                        RecalculateZSMCSpent();
                    }
                    RecalculateSMCSupply(1);
                }
                ...
                // SMC: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of SMC Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart SMC Core to complete") << "\n";
        ...
        zwalletMain = new CzSMCWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zSMCWallet
        uiInterface.InitMessage(_("Syncing zSMC wallet..."));
...
//    }
//
//    nAnonymizePivxAmount = GetArg("-anonymizesmcamount", 0);
...
//    if (nAnonymizePivxAmount < 2) nAnonymizePivxAmount = 2;
    ...
    LogPrintf("Anonymize SMC Amount %d\n", nAnonymizePivxAmount);
    ...
    /* Denominations
       ...
       For example:
       1SMC+1000 == (.1SMC+100)*10
       10SMC+10000 == (1SMC+1000)*10
    */
