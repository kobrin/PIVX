...
#if defined(NDEBUG)
#error "SMC cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 8 * 60 * 60; //8 hr
...
{
    //Check to see if the zSMC is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zSMC spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zSMC without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zSMC spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zSMC spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zSMC spend tx %s already in block %d",
            ...
            // Check that zSMC mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    int64_t nSubsidy = 0;
    if (nHeight == 0) {
        nSubsidy = 35000000 * COIN; //block1
    } else if (nHeight < 6721 && nHeight > 0) {
        nSubsidy = 0 * COIN; //block2
    } else if (nHeight < 93121 && nHeight >= 6722) {
        nSubsidy = 50 * COIN; //block3
    } else if (nHeight < 179521 && nHeight >= 93122) {
        nSubsidy = 30 * COIN; //block4
    } else if (nHeight < 265921 && nHeight >= 179522) {
        nSubsidy = 10 * COIN;  //block5
    } else if (nHeight < 352321 && nHeight >= 265922) {
        nSubsidy = 5 * COIN; //block6
    } else if (nHeight >= 352322) {
        nSubsidy = 1 * COIN; //block7
    } 
    return nSubsidy;
}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZSMCStake)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from SMC
...
{
    RenameThread("smc-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZSMCMinted()
{
...
void RecalculateZSMCSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zSMC supply
...
bool RecalculateSMCSupply(int nHeightStart)
{
...
bool UpdateZSMCSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateSMCSupply(Params().Zerocoin_StartHeight());
    }

    //Track zSMC money supply in the block index
    if (!UpdateZSMCSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zSMC supply for block=%s height=%d", __func__,
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert SMC to zSMC
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zSMC spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zSMC serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zSMC stake block is less than allowed start height", __func__);

    if (CZPivStake* zSMC = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zSMC->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zSMC stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zSMC stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zSMC->GetValue()));
        if (nChecksum200 != zSMC->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zSMC->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZSMC() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zSMC fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zSMC mints and %d zSMC spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // SMC: We use certain sporks during IBD, so check to see if they are
